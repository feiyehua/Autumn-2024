 ...
 * @LastEditTime : 2024-11-09 13:01:22
 * @LastEditors  : FeiYehua
 * @Description  : 
 * @FilePath     : T536792.cpp
 * @     © 2024 FeiYehua
 */
#include<bits/stdc++.h>
using namespace std;
...
int main()
{
    freopen("/Users/xiong/Documents/CPP/Autumn-2024/MX-S5/kingdom/kingdom3.in","r",stdin);
    n=fr(),m=fr(),q=fr();
    int lastOne=-1;
    for(int i=1;i<=n;i++)
    {
    ...
    for(int i=1;i<=q;i++)
    {
        ...
        else
        {
            ...
            for (long long j = 1; j <= k; j++)
            {
                ...
                {
                    //前j次为一个循环
                    ans=(((ans-start)%MOD)*((k/j-1)%MOD)%MOD)+start;
                    j=0,k=k%j;
                }
            }
            printf("%lld\n",ans);
